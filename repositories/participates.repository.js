...
class ParticipateRepository {
  ...
  findOneParty = async ({ userId, meetingId }) => {
    ...
    const findOneParty = await Participates.findOne({
  ...
  upPartyCount = async ({ meetingId }) => {
    await Meetings.increment({ participateCount: 1 }, { where: { meetingId } });
  };
  ...
  joinParty = async ({ userId, meetingId, nickname }) => {
    ...
    const joinParty = await Participates.create({
      userId,
      meetingId,
      nickname,
    });
  ...
  cancelParty = async ({ userId, meetingId }) => {
    const cancelParty = await Participates.destroy({
      where: { userId, meetingId },
    });
    return cancelParty;
  };
